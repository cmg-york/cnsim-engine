# Configuring CNSim-Engine

## Overview

CNSim-Engine reads all runtime parameters from a Java `Properties` file loaded through the `ca.yorku.cmg.cnsim.engine.config.Config` class. Every `Config.getPropertyXxx(key)` call in the engine sources is keyed by a string defined below; every `Config.hasProperty(key)` call marks an optional key whose absence produces default behavior rather than an error.
...
## Gap Analysis: `application.properties` vs. Engine Code

The shipped example at `src/main/resources/application.properties` covers most common keys but diverges from the current engine code in a few places. The lists below are derived by comparing the properties file side-by-side with every `Config.getPropertyXxx(...)` / `Config.hasProperty(...)` call in the engine sources.

### Consumed by the engine but absent from `application.properties`

These keys are read from code but have no entry in the shipped example — supply them explicitly in your own configuration if you want the corresponding behavior:

* `sim.terminate.atTime` — mandatory; `SimulatorFactory#buildSimulator` will fail without it.
* `sim.experimentalLabel` — optional; no label in filenames when absent.
* `reporter.reportingWindow` — mandatory when periodic reports are enabled (note: the properties file has the lookalikes `sim.reportingWindow` and `sim.reporting.window`, but neither is read by the engine).
* `reporter.reportSampleTransactionsOnly`.
* `reporter.events.belief`, `reporter.events.containerArrival`, `reporter.events.containerValidation`, `reporter.events.newTransactionArrival`, `reporter.events.transactionPropagation`, `reporter.events.seedUpdate`.
* `reporter.enablePeriodicReports`, `reporter.enableTimeAdvancementReports`.
* `reporter.beliefReportInterval`, `reporter.beliefReportOffset`.
* `workload.hasConflicts`, `workload.conflicts.dispersion`, `workload.conflicts.likelihood`.
* `workload.hasDependencies`, `workload.dependencies.dispersion`, `workload.dependencies.countMean`, `workload.dependencies.countSD`, `workload.dependencies.mandatory`.
* `workload.sampler.seed.updateTransaction` — only `workload.sampler.seed` and `workload.sampler.seed.updateSeed` are in the example; this third key is also read by `StandardTransactionSampler#LoadConfig`.

### Present in `application.properties` but not consumed by the engine

These keys appear in the shipped example but are never read by the engine in this module. They may be historical, consumed only by downstream protocol modules (`cnsim-bitcoin`, `cnsim-tangle`), or stale:

* `sim.numSimulations`, `sim.maxNodes`, `sim.maxTransactions`, `sim.numofSim`, `sim.reportingWindow`, `sim.reporting.window` — no `Config.getProperty\*` consumer in the engine. The correct reporting-window key is `reporter.reportingWindow`.
* `net.numOfHonestNodes`, `net.numOfMaliciousNodes` — not referenced in the engine.
* `worlkoad.targetTransaction` — note the typo (`worlkoad` vs `workload`); no engine consumer.
* `reporter.reportBeliefEvents` — the engine reads `reporter.events.belief` (under the `reporter.events.\*` family), not this key.
* `node.createMaliciousNode`, `node.maliciousPowerByRatio`, `node.maliciousHashPower`, `node.maliciousRatio` — consumed by the Bitcoin protocol module, not by the engine.
* `bitcoin.maxBlockSize`, `bitcoin.minSizeToMine`, `bitcoin.minValueToMine` — Bitcoin protocol module.
* `tangle.alpha`, `tangle.numOfFounders`, `tangle.numOfParticles`, `tangle.startingCutset`, `tangle.wMaxPercentile`, `tangle.wMinPercentile`, `tangle.randomSeed` — Tangle protocol module.
* `sampler.useFileBasedSampler` — commented-out placeholder, not read.

### Keys Referenced Through `hasProperty` Only

These keys are checked for presence (their value is used only to select a code path, not parsed):

* `workload.hasConflicts`, `workload.hasDependencies` — also read with `getPropertyBoolean` in `SimulatorFactory`, but `Event\_NewTransactionArrival` only calls `hasProperty(...)` to decide the `Reporter.addTx(...)` column layout.
* `sim.experimentalLabel`, `reporter.reportBeliefsShort`, `workload.dependencies.mandatory`, `workload.sampler.seed`, `workload.sampler.seed.updateSeed`, `workload.sampler.seed.updateTransaction`, `net.sampler.seed`, `net.sampler.seed.updateSeed` — guarded by `hasProperty` before use, so absence is a valid state.

## Parsing Helpers

`Config` exposes two utilities for array-valued properties, used with the braced-list syntax:

* `Config.parseStringToArray("{1,2,3}")` → `long\[]{1,2,3}`. Used for `node.sampler.seed`, `node.sampler.seedUpdateTimes`, `workload.sampleTransaction`.
* `Config.parseStringToBoolean("{true,false}")` → `boolean\[]{true,false}`. Used for `node.sampler.updateSeedFlags`.

`Config.getOptionalPropertyBoolean(key)` returns `false` when the key is absent — this is the mechanism behind every "optional boolean" toggle listed above.
