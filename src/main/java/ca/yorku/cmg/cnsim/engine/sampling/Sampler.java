...
public class Sampler {
     ...
     * The result is always positive using absolute value. 
     *
	 * <p><b>JML Contract:</b></p>
	 * <pre>{@code
	 ...
	 * @param random The {@linkplain Random} number generator to use
	 * @return The generated random value following the Gaussian distribution. If negative, the absolute value is returned.
	 * @throws ArithmeticException if {@code deviation < 0}
	 * @throws NullPointerException if {@code random} is null
     */
    public float getGaussianPos(float mean, float deviation, Random random) {
		getGaussian_pre(mean, deviation, random);

    	float gaussianValue = Math.abs(mean + (float) random.nextGaussian() * deviation);
	...
	public float getPositiveGaussian(float mean, float deviation, Random random) {
		...
		return getGaussianPos(mean, deviation, random);
	}
